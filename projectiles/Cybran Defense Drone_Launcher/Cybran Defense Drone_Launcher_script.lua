-----------------------------------------------------------------------------
--  File     : /mods/Future Battlefield Pack/projectiles/Cybran Defense Drone_Launcher/Cybran Defense Drone_Launcher_script.lua
--
...
drone_launcher01 = Class(CElectronBolterProjectile) {
	...
	SpawnDrone = function(self)  
        if not self:BeenDestroyed() and self:GetLauncher() then 
            ...
            local drone = CreateUnit('URA0002', launcher:GetArmy(), projLoc[1], projLoc[2], projLoc[3], projOri[1], projOri[2], projOri[3], projOri[4], 'Air')                         
