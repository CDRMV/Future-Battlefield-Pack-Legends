-----------------------------------------------------------------------------
--  File     : /mods/Future Battlefield Pack/projectiles/Cybran Assault Drone_Launcher/Cybran Assault Drone_Launcher_script.lua
--
...
drone_launcher01 = Class(CElectronBolterProjectile) {
	...
	SpawnDrone = function(self)  
        if not self:BeenDestroyed() and self:GetLauncher() then 
            ...
            local drone = CreateUnit('URA0003', launcher:GetArmy(), projLoc[1], projLoc[2], projLoc[3], projOri[1], projOri[2], projOri[3], projOri[4], 'Air')                         
                                           
            ### Sets the Carrier unit as the drones parent 
            drone:SetParent(launcher, 'URB1200','URB2200') 
